...
import { FileText, Download, Printer, CheckCircle, AlertCircle, Loader, FileSignature } from 'lucide-react';
...
<body>
    ...
    <div class="info-grid">
        ...
        <div>${order.items?.[0]?.barType || 'Disclosed'} x ${order.items?.[0]?.length || ''}</div>
        
        <div class="info-label">Payment Terms:</div>
        <div>${order.paymentTerms || '30 Days from delivery to yard'}</div>
        
        <div class="info-label">Delivery Terms:</div>
        <div>${order.deliveryTerms || 'Delivery Duty paid - unloading by purchaser'}</div>
        
        <div class="info-label">Total Quantity:</div>
        <div>${order.items?.[0]?.totalWeight || order.items?.[0]?.quantity || 0} tonnes</div>
        
        <div class="info-label">Quantity Tolerance:</div>
        <div>${order.quantityTolerance || '+/- 10%'}</div>
        
        <div class="info-label">Invoicing Basis:</div>
        <div>${order.invoicingBasis || 'Theoretical Weight'}</div>
        ...
        <div>Mill's Standard for Export</div>
        ...
        <div>Commercial Invoice<br>
        Certificate of Origin<br>
        Mill Test Certificates<br>
        ACRS Certification</div>
    </div>
    ...
    <div style="page-break-before: always; margin-top: 40px;">
        ...
        <table class="product-table">
            <thead>
                <tr>
                    <th>Description of Goods<br>AS/NZS 4671 : 2019 Grade 500 N Reinforcing Bar ACRS</th>
                    <th>Price<br>(AUD per tonne)</th>
                    <th>Qty<br>(tonnes)</th>
                    <th>Amount<br>(AUD)</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td class="description-cell">${order.items?.[0]?.barType || 'N16'} x ${order.items?.[0]?.length || '6'}m</td>
                    <td>${order.items?.[0]?.pricePerTonne?.toLocaleString('en-AU', { minimumFractionDigits: 0 }) || '0'}</td>
                    <td>${order.items?.[0]?.totalWeight || order.items?.[0]?.quantity || '0'}</td>
                    <td>${order.subtotal?.toLocaleString('en-AU', { minimumFractionDigits: 2 }) || '0.00'}</td>
                </tr>
                <tr class="total-row">
        ...
        <div class="payment-terms">
            STRICTLY 30 DAYS FROM ARRIVAL TO YARD
        </div>
...
<body>
    ...
    <div class="info-grid">
        <div class="info-label">Product Description:</div>
        <div>${order.items?.[0]?.barType || 'N16'} x ${order.items?.[0]?.length || 6}m</div>
        <div class="info-label">Payment Terms:</div>
        <div>${order.paymentTerms || '30 Days from delivery to yard'}</div>
        <div class="info-label">Delivery Terms:</div>
        <div>${order.deliveryTerms || 'Delivery Duty paid - unloading by purchaser'}</div>
        ...
        <div class="info-label">Packing:</div>
        <div>${order.packing || "Mill's Standard for Export"}</div>
        <div class="info-label">Shipment Date:</div>
        <div>${order.estimatedDelivery ? `Delivery by ${formatLongDate(order.estimatedDelivery)}` : order.shipmentDetails || 'TBD'}</div>
        <div class="info-label">Documentation:</div>
        <div>${order.documentation || 'Commercial Invoice<br>Certificate of Origin<br>Mill Test Certificates<br>ACRS Certification'}</div>
    </div>
      ...
      <div className="bg-white/95 backdrop-blur-xl rounded-2xl max-w-md w-full p-6 border border-white/30 shadow-2xl">
        ...
        <div className="space-y-4 mb-6">
          <div className="bg-gradient-to-r from-blue-50/50 to-purple-50/50 rounded-lg p-4 border border-blue-200/30">
            ...
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm text-blue-700/70">Product:</span>
              <span className="font-medium text-blue-800">
                {order.items?.[0]?.barType} × {order.items?.[0]?.length}m
              </span>
            </div>
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm text-blue-700/70">Quantity:</span>
              <span className="font-medium text-blue-800">{order.items?.[0]?.totalWeight || order.items?.[0]?.quantity} tonnes</span>
            </div>
