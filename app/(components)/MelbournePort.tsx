...
import { 
  ...
  ArrowUp, ArrowDown, Timer, Building, Globe, Truck, FileText, Container, Palmtree, Car, Package
} from 'lucide-react';
...
interface MelbournePortData {
  ...
  data: {
    ...
    raw_data: {
      ...
      vicports_movements: VicPortsMovement[];
...
export default function MelbournePortDashboard() {
  const [data, setData] = useState<MelbournePortData | null>(null);
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'inport' | 'arrivals' | 'vicports' | 'history' | 'analytics'>('overview');
  const [analyzing, setAnalyzing] = useState(false);

  // Fetch Melbourne vessel data
  const fetchMelbourneData = async (withAnalysis = false) => {
    setLoading(true);
    setError(null);
    ...
    try {
      const url = withAnalysis 
        ? '/api/scrape-melbourne-vessels?analyze=true'
        : '/api/scrape-melbourne-vessels';
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => fetchMelbourneData(), 15 * 60 * 1000);
  ...
  try {
    ...
    // Handle raw scraped format - assume it's UTC, convert to Melbourne
    const [datePart, timePart] = dateString.split(' ');
    if (!timePart) return dateString;
    
    // Parse as UTC and convert to Melbourne time
  ...
  const getFilteredVicPortsMovements = () => {
    if (!data) return [];
    return data.data.raw_data.vicports_movements.filter(movement =>
      movement.vessel.toLowerCase().includes(searchTerm.toLowerCase()) ||
      movement.berth.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-slate-100 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardContent className="flex flex-col items-center space-y-4 p-6">
            ...
            <p className="text-center text-muted-foreground">Loading Melbourne Port data...</p>
            <p className="text-center text-sm text-muted-foreground">
              Scraping MyShipTracking, VicPorts, and port call history...
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-slate-100 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardContent className="text-center py-8">
            ...
            <Button onClick={() => fetchMelbourneData()} className="mt-4">
              Try Again
            </Button>
          </CardContent>
        </Card>
          ...
          <CardHeader className="pb-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <CardTitle className="text-2xl md:text-3xl">Port Melbourne Vessel Dashboard</CardTitle>
                  <CardDescription>Australia's largest container port and gateway to Victoria</CardDescription>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-3">
                <Button
                  onClick={() => fetchMelbourneData()}
                  disabled={loading}
                  variant="outline"
                ...
                <Button
                  onClick={() => fetchMelbourneData(true)}
            ...
            <div className="text-sm text-muted-foreground pt-2 border-t">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  {data.data.summary.sources_active.length}/4
                </div>
              </div>
            </div>
          </CardHeader>

          {/* Melbourne Stats Cards */}
          <CardContent>
            <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
              ...
              <Card className="bg-gradient-to-r from-slate-500 to-slate-600 text-white border-0">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-slate-100 text-sm">Expected</p>
              ...
              <Card className="bg-gradient-to-r from-purple-500 to-purple-600 text-white border-0">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-purple-100 text-sm">VicPorts</p>
                      <p className="text-2xl font-bold">{processedData.vicPortsMovements}</p>
                    </div>
              ...
              <Card className="bg-gradient-to-r from-green-500 to-green-600 text-white border-0">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-green-100 text-sm">Large Vessels</p>
                      <p className="text-2xl font-bold">
                        {data.data.raw_data.in_port_vessels.filter(v => parseInt(v.size) > 200).length}
                      </p>
                    </div>
                    <Ship className="h-8 w-8 text-green-200" />
                  </div>
                </CardContent>
              </Card>
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)} className="w-full">
          ...
          <TabsContent value="overview" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    {processedData.busyBerths.slice(0, 5).map((berth, idx) => (
                      <div key={idx} className="flex justify-between items-center">
                        <span className="text-sm font-medium">{berth}</span>
                        <Badge variant="outline">Active</Badge>
                      </div>
                    ))}
                    {processedData.busyBerths.length === 0 && (
                      <p className="text-sm text-muted-foreground">No berth data available</p>
                    )}
                  </div>
                </CardContent>
              </Card>
              ...
              {/* Melbourne Port Icon */}
              <Card className="bg-gradient-to-r from-blue-500 to-slate-600 text-white">
                <CardHeader>
                ...
                <CardContent>
                  <div className="space-y-2">
                    <p className="text-blue-100 text-sm">Australia's largest container port and gateway to Victoria</p>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-blue-100">Port Activity</span>
                      <Badge variant="secondary" className="bg-white/20 text-white border-white/30">
                        {processedData.inPortCount > 50 ? 'High' : processedData.inPortCount > 25 ? 'Moderate' : 'Normal'}
                      </Badge>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-blue-100">Container Operations</span>
                      <Badge variant="secondary" className="bg-white/20 text-white border-white/30">
                        24/7
                      </Badge>
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div>
                        <h4 className="font-medium text-sm mb-2">Traffic Patterns</h4>
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.port_activity_analysis?.traffic_patterns || 'No data available'}
                        </p>
                      </div>
                      <div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div>
                        <h4 className="font-medium text-sm mb-2">Size Distribution</h4>
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.vessel_composition?.size_distribution || 'No data available'}
                        </p>
                      </div>
                      <div>
                        <h4 className="font-medium text-sm mb-2">Flag State Analysis</h4>
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.vessel_composition?.flag_state_analysis || 'No data available'}
                        </p>
                      </div>
                    </div>
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div>
                        <h4 className="font-medium text-sm mb-2">Berth Utilization</h4>
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.infrastructure_analysis?.berth_utilization || 'No data available'}
                        </p>
                      </div>
                      <div>
                        <h4 className="font-medium text-sm mb-2">Terminal Efficiency</h4>
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.infrastructure_analysis?.terminal_efficiency || 'No data available'}
                        </p>
                      </div>
                      <div>
                        <h4 className="font-medium text-sm mb-2">Capacity Constraints</h4>
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.infrastructure_analysis?.capacity_constraints || 'No data available'}
                        </p>
                      </div>
                    </div>
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      ...
                      <div>
                        <h4 className="font-medium text-sm mb-2">Cargo Flow Patterns</h4>
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.trade_insights?.cargo_flow_patterns || 'No data available'}
                        </p>
                      </div>
                      <div>
                        <h4 className="font-medium text-sm mb-2">Economic Indicators</h4>
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.trade_insights?.economic_indicators || 'No data available'}
                        </p>
                      </div>
                    </div>
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div>
                        <h4 className="font-medium text-sm mb-2">Operational Improvements</h4>
                        <ul className="text-sm text-muted-foreground space-y-1">
                          {processedData.analysis.strategic_recommendations?.operational_improvements?.map((rec: string, idx: number) => (
                      ...
                      <div>
                        <h4 className="font-medium text-sm mb-2">Efficiency Gains</h4>
                        <ul className="text-sm text-muted-foreground space-y-1">
                          {processedData.analysis.strategic_recommendations?.efficiency_gains?.map((gain: string, idx: number) => (
                            <li key={idx} className="flex items-start gap-2">
                              <span className="text-green-500 mt-1">•</span>
                              {gain}
                            </li>
                          )) || <li>No efficiency recommendations available</li>}
                        </ul>
                      </div>
                    </div>
                  </CardContent>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {processedData.analysis.melbourne_insights?.unique_characteristics || 'No data available'}
                        </p>
                      </div>
                      <div>
                        <h4 className="font-medium text-sm mb-2">Key Strengths</h4>
                        <ul className="text-sm text-muted-foreground space-y-1">
                          {processedData.analysis.melbourne_insights?.strengths?.map((strength: string, idx: number) => (
                            <li key={idx} className="flex items-start gap-2">
                              <span className="text-green-500 mt-1">✓</span>
                              {strength}
                            </li>
                      ...
                      <div>
                        ...
                        <ul className="text-sm text-muted-foreground space-y-1">
                          {processedData.analysis.melbourne_insights?.growth_opportunities?.map((opp: string, idx: number) => (
                            <li key={idx} className="flex items-start gap-2">
                              <span className="text-blue-500 mt-1">→</span>
                              {opp}
                            </li>
              ...
              <Card>
                <CardContent className="text-center py-12">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Click "AI Analysis" to generate comprehensive Melbourne port insights
                  </p>
                  <Button onClick={() => fetchMelbourneData(true)} disabled={analyzing}>
                    <BarChart3 className={`mr-2 h-4 w-4 ${analyzing ? 'animate-pulse' : ''}`} />
